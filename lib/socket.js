...
var socketLib = function (io, sessionStore) {
    ...
    socket.on('init', function () {
      if (socket.handshake.headers.sessions) {
        ...
        if (socket_user_hash[user.uid] && (socket.id != socket_user_hash[user.uid].id)) {
          socket_user_hash[user.uid].emit('loginFail', '已在其他地方登录，请重新登录！');
          socket_user_hash[user.uid].disconnect();
        }
    ...
    socket.on('audioMsg', function (msg, to) {
      var mType = 'group';
      if (!socket.nickname) {
        socket.emit('loginFail', '请先登录！');
      } else {
        if (to === 'all') {
          socket.broadcast.emit('newAudioMsg', socket.nickname, msg, to);
        } else {
          socket.to(socket_user_hash[to].id).emit('newAudioMsg', socket.nickname, msg, socket.user.uid);
        }
      }
    });
